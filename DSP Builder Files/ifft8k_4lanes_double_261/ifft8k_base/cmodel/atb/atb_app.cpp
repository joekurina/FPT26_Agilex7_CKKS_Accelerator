...
int main(int argc, char** argv)
{
    ...
    if (!s_has_error && success)
    {
        if (run_individual_atbs)
        {
            csl::info("[ifft8k_base_DUT] Success! All individual software model ATB results matched the Simulink simulation results.");
        }
        else
        {
            csl::info("[ifft8k_base_DUT] Success! Device-level software model ATB results matched the Simulink simulation results.");
        }
    }
    else if (s_has_error)
    {
        csl::error("[ifft8k_base_DUT] An error occurred before the software model ATB could complete.");
    }
    else
    {
        if (run_individual_atbs)
        {
            csl::error("[ifft8k_base_DUT] Failed! At least one individual ATB result did not match the Simulink simulation results.");
        }
        else
        {
            csl::error("[ifft8k_base_DUT] Failed! Device-level software model ATB results did not match the Simulink simulation results.");
        }
