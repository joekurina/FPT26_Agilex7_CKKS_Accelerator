...
void* mp_no_malloc(size_t size)
{
    fatal("malloc called after MP allocations were disabled");
}

void mp_no_free(void* ptr, size_t size)
{
...
inline void set_disabled_mp_allocators() noexcept
{
    // mp types may still realloc
...
inline void not_n(mp_int& result, const mp_int& value, int bit_width) noexcept
{
    ...
    for (int i = 0; i < bit_width; ++i)
    {
        mpz_combit(result.get(), static_cast<mp_bitcnt_t>(i));
    }
}
...
#ifdef CSL_USE_MPFR
CSL_NO_INLINE float fp_mul_impl(const float& x, const float& y) noexcept
{
...
{
    if (has_signed_in && csl::test_bit(bit_width_in - 1, q))
    {
        csl::set_upper(q, bit_width_in);
    }
    else
    {
        csl::mask_lower(q, q, bit_width_in);
    }
    if (has_signed_out && csl::test_bit(bit_width_out - 1, q))
    {
        csl::set_upper(q, bit_width_out);
    }
    else
    {
        csl::mask_lower(q, q, bit_width_out);
    }
}
...
inline void step_fp_mul(int64_t& iq, int64_t ia, int64_t ib) noexcept
{
    float a = sanitize_f32(ia);
    float b = sanitize_f32(ib);
    set(iq, f32_to_i64(sanitize_f32(fp_mul_impl(a, b))));
}
...
CSL_NO_INLINE bool get_sorted_lookup_value(const uint64_t* keys, const int64_t* values, uint64_t n, uint64_t key,
                                           int64_t& out) noexcept
{
    uint64_t l = 0;
    uint64_t r = n - 1;
...
CSL_NO_INLINE void step_lookup(const int64_t* values, uint64_t n, uint64_t offset, int64_t& iq, int64_t ia_in)
{
    uint64_t ia_u64;
...
CSL_NO_INLINE void step_lookup_hash(const int64_t* values, uint64_t n, uint64_t mult, int64_t& iq, int64_t ia_in)
{
    uint64_t ia_u64;
    memcpy(&ia_u64, &ia_in, sizeof(uint64_t));
...
CSL_NO_INLINE void step_fp_mult_acc(int64_t& iacc, int64_t control, int64_t& iq, int64_t ix, int64_t iy) noexcept
{
    step_fp_mul(iq, ix, iy);
    if (control == 0)
    {
        // acc = (x*y)
        set(iacc, iq);
...
inline void step_logical_finalize_inv(mp_int& q, int bit_width_in, int bit_width_out, bool has_signed_in, bool has_signed_out) noexcept
{
    csl::not_n(q, q, bit_width_in);
    if (has_signed_in && csl::test_bit(bit_width_in - 1, q))
    {
...
CSL_NO_INLINE bool get_sorted_lookup_value(const uint64_t* keys, const mp_int* values, uint64_t n, uint64_t key, mp_int& out) noexcept
{
    uint64_t l = 0;
    uint64_t r = n - 1;
    while (l <= r)
...
CSL_NO_INLINE void step_lookup(const int64_t* values, uint64_t n, uint64_t offset, mp_int& iq, int64_t ia_in)
{
    uint64_t ia_u64;
    memcpy(&ia_u64, &ia_in, sizeof(uint64_t));
    uint64_t ia = ia_u64 - offset;
...
CSL_NO_INLINE void step_lookup(const uint64_t* values, const mp_int_info* infos, uint64_t n, uint64_t offset, int64_t& iq,
                               int64_t ia_in, mp_temp_allocator& alloc)
{
    uint64_t ia_u64;
    memcpy(&ia_u64, &ia_in, sizeof(uint64_t));
    uint64_t ia = ia_u64 - offset;
...
CSL_NO_INLINE void step_lookup_with_valid(const uint64_t* values, const mp_int_info* infos, uint64_t n, uint64_t offset, int64_t& iq,
                                          int64_t& ivalid, int64_t ia_in, mp_temp_allocator& alloc)
{
    uint64_t ia_u64;
    memcpy(&ia_u64, &ia_in, sizeof(uint64_t));
    uint64_t ia = ia_u64 - offset;
...
CSL_NO_INLINE void step_lookup_with_valid(const uint64_t* values, const mp_int_info* infos, uint64_t n, uint64_t offset, int64_t& iq,
                                          mp_int& ivalid, int64_t ia_in, mp_temp_allocator& alloc)
{
    uint64_t ia_u64;
    memcpy(&ia_u64, &ia_in, sizeof(uint64_t));
    uint64_t ia = ia_u64 - offset;
...
CSL_NO_INLINE void step_lookup_hash(const int64_t* values, uint64_t n, uint64_t mult, mp_int& iq, int64_t ia_in)
{
    uint64_t ia_u64;
    memcpy(&ia_u64, &ia_in, sizeof(uint64_t));
    uint64_t ia = (ia_u64 * mult) % n;
...
CSL_NO_INLINE void step_lookup_hash_with_valid(const uint64_t* values, const mp_int_info* infos, uint64_t n, uint64_t mult,
                                               int64_t& iq, int64_t& ivalid, int64_t ia_in, mp_temp_allocator& alloc)
{
    uint64_t ia_u64;
    memcpy(&ia_u64, &ia_in, sizeof(uint64_t));
    uint64_t ia = (ia_u64 * mult) % n;
...
CSL_NO_INLINE void step_lookup_hash_with_valid(const uint64_t* values, const mp_int_info* infos, uint64_t n, uint64_t mult,
                                               int64_t& iq, mp_int& ivalid, int64_t ia_in, mp_temp_allocator& alloc)
{
    uint64_t ia_u64;
    memcpy(&ia_u64, &ia_in, sizeof(uint64_t));
    uint64_t ia = (ia_u64 * mult) % n;
...
CSL_NO_INLINE void step_fp_acc(int64_t& iacc, int64_t control, int64_t& iq, int64_t ix, mp_temp_allocator& alloc) noexcept
{
    alloc.push();
    float x = sanitize_f32(ix);

    mp_float& mp_acc = alloc.alloc_float(24);
    if (control == 0)
    {
        // acc = x
        mpfr_set_d(mp_acc.get(), static_cast<double>(x), GMP_RNDN);
    }
    else
    {
        mp_float& mp_x = alloc.alloc_float(24);
        mpfr_set_d(mp_x.get(), static_cast<double>(x), GMP_RNDN);

        float acc = sanitize_f32(iacc);
        mpfr_set_d(mp_acc.get(), static_cast<double>(acc), GMP_RNDN);
...
CSL_NO_INLINE void step_fp_acc(mp_int& iacc, int64_t control, int64_t& iq, int64_t ix, mp_temp_allocator& alloc) noexcept
{
    alloc.push();
    float x = sanitize_f32(ix);
...
#endif // CSL_USE_MPFR
CSL_NO_INLINE void step_fp_mult_acc(mp_int& iacc, int64_t control, int64_t& iq, int64_t ix, int64_t iy) noexcept
{
    step_fp_mul(iq, ix, iy);
    if (control == 0)
    {
